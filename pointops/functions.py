...
def uniform_sphere(nsamples=5000,radius=1,batch_size=32):
    batch = []
    for b in range(0,batch_size):
        phi = random.uniform(0, 2 * pi, nsamples)
        theta = arccos(random.uniform(-1, 1, nsamples))
        r = random.uniform(0, radius, nsamples)
        x = []
        y = []
        z = []
    ...
    if torch.cuda.is_available():
        return torch.Tensor(batch).permute([0,2,1]).cuda()
    else:
        return torch.Tensor(batch).permute([0,2,1])

def normal_sampling(ratio=4,mean=0,std=0.01,batch_size=32,points=1024,cuda=True):
    if cuda:
        return torch.normal(mean,std,size=(batch_size,3,points,ratio)).cuda()
    else:
        return torch.normal(mean, std, size=(batch_size, 3, points, ratio))
if __name__ == '__main__':
    # p = uniform_sphere()
    # print(p.shape)
    x = torch.randn(1,3,2)

    diff = torch.normal(0,0.01,size=(1,3,2,4))

    print(x)
    print(x.unsqueeze(-1).repeat(1,1,1,4) + diff)